...
class CSGNN(nn.Module):
    ...
    def __init__(self, icosphere, radius_lvls, gconv_type, gconv_dims, gconv_depths, outblock_hidden_dims,
                       ...
                       pad=0, dropout=0, residual=True, gcn_norm=False, classify=False, debug=False):
        ...
        for i in range(self.max_level):
            ...
            conv_block = ConvBlock(g, gconv_type, gconv_in_dim, gconv_h_dim, gconv_out_dim, gconv_depth, self.activation, gcn_norm, residual, debug=debug)
            ...
            conv_radial_block = ConvRadialBlock(conv_radial_in_dim, conv_radial_out_dim, n_radial_convs, k_radial, pad=pad, debug=debug)
        ...
        self.output = OutputBlock(outblock_in_dim, outblock_hidden_dims, out_dim, dropout, classify=classify, debug=debug)

    def forward(self, x, mask):
        """
        Input: [n_batch, n_atoms, n_radius, n_mesh, n_feats]
        """

        is_eval = not self.training
        n_batch, n_atoms, n_radius, n_mesh, n_features = x.size()
        x = x.permute((3, 0, 1, 2, 4)) # [n_mesh, n_batch, n_atoms, n_radius, n_features]
        ...
        x = torch.squeeze(x, dim=0) # n_batch, n_atoms, n_features
    ...
    def __init__(self,
        ...
        for i in range(n_layers):
            ...
            if self.gconv_type == "gcn": 
                self.conv_layers.append(GCNLayer(self.g, curr_in_feats, curr_out_feats, activation, normalize=gcn_norm, batch_norm=BATCH_NORM, residual=residual, bias=GC_BIAS))
...
class Pool(nn.Module):
    ...
    def pool(self, x):
        ...
        (+1 from self vertex)
        """
        n_vertices, n_batch, n_atoms, n_radius, n_feats = x.shape
        base_pool_idx = self.base_pool_idx.reshape(-1)
        out_base = x[base_pool_idx, ...]
        out_base = out_base.reshape((-1, 6, n_batch, n_atoms, n_radius, n_feats))
        ...
        if self.rest_pool_idx is not None:
            ...
            out_rest = out_rest.reshape((-1, 7, n_batch, n_atoms, n_radius, n_feats))
            if self.pool_type == "sum":
...
class ConvRadialBlock(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, n_convs, kernel, pad=0, debug=False):
    ...
    def forward(self, x, is_eval=False):
        V, B, N, R_in, F_in = x.shape
        x = x.permute((0, 1, 2, 4, 3)) # V, B, N, F_in, R_in
        x = x.reshape(V*B*N, F_in, R_in)
        ...
        x = x.reshape(V, B, N, F_out, R_out)
        x = x.permute((0, 1, 2, 4, 3)) # V, B, N, R_out, F_out
        return x

    def __repr__(self):
        return '{}(pad={}, k={}, layers={})'.format(self.__class__.__name__,
...
class OutputBlock(nn.Module):
    ...
    def __init__(self, f, h_dims, out_dim, dropout, classify=True, debug=False):
        """ f:         input filters
    ...
    def forward(self, x, mask):
        ...
        Input: [B x N x F]
        """
        n_batch, n_atoms, n_feats = x.shape
        x = x.reshape(n_batch*n_atoms, n_feats)
        ...
        x = x.reshape(n_batch*n_atoms, 1, -1)
        x = self.conv1d(x)
        x = self.nonlinearity(x)
        x = torch.mean(x, axis=1)

        # pool non-null atoms
        x = x.reshape(n_batch, n_atoms, -1)
        nnz_atoms = torch.sum(mask, axis=-1).unsqueeze(1)
        mask = torch.unsqueeze(mask, 2).float()
        # dos
        x = torch.sum(x * mask, dim=1) # n_batch, n_bins
        x /= nnz_atoms
        # fdos
        x2 = 0.1*torch.cumsum(x, -1)
